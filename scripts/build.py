...
# The addon must stay a loader-delivered Lua resource: no DLL loading, no code
# or memory mutation, no system-wide hooks. GameGuard trips on those, and the
# mod does not need them.
FORBIDDEN_APIS = (
...
def main():
    ...
    source = read_source()
    tests = ''
    for name in ('test_detector.lua', 'test_install.lua', 'test_platform.lua'):
        test_path = ROOT / 'tests' / name
        output, env = lua([test_path, WORKSPACE, SOURCE])
        tests += output
    ...
    report = {
        ...
        'description': 'Click a menu scrollbar track to move the thumb there, and drag the '
                       'thumb to scroll with the pointer. Requires Bingus Shared Loader v15 '
                       'or newer / API 1.',
        'requires': [{'name': 'Bingus Shared Loader', 'api': 1, 'revision': 'loader-v15'}],
        'mechanism': {
            'input': 'left-button press, cursor position',
            'detection': 'screen strip capture; uniform grey vertical thumb with dark '
                         'track beside it; the pointer sprite is masked and bridged',
            'output': 'SendInput mouse-wheel notches at the cursor, sent on the frame that '
                      'produces them (no pacing queue); settle corrections after a click are '
                      'bounded by max_corrections and applied only when the measured step '
                      'shrinks the remaining error',
            'follow_up': 'none after the click; the thumb is verified once it has stopped',
            'writes_game_memory': False, 'patches_executable_memory': False,
            'custom_dll': False,
        },
        ...
        'runtime_verified': False, 'status': 'offline_verified_in_game_pending',
    ...
    print(tests.strip())
    print('Built ' + str(release) + '; in-game validation pending.')
